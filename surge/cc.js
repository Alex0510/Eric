...
(async () => {
  try {
    ...
    // 如果 BoxJS 密码为空，则保存默认Base64编码后的密码到 BoxJS
    if (!boxjsPassword) {
            ...
            try {
              ...
              if (dataObj && dataObj.data && dataObj.data.length > 0) {
                ...
                if (fetchedData.last_operate !== undefined && fetchedData.distance !== undefined) {
                  ...
                  dataObj.data[0].is_hide_distance = 0;
                  dataObj.data[0].is_hide_last_operate = 0;

                  console.log("Modified fetched data:", JSON.stringify(dataObj, null, 2));
                } else {
    ...
    $done({ body: JSON.stringify({ error: "Unexpected error occurred" }) });
  }
})();
