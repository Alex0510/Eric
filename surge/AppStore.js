...
async function main() {
    ...
    if (purchaseType === '本体') {
        ...
        priceInfo = priceElement ? priceElement[1].trim() : null;
    } else if (purchaseType === '内购') {
        ...
        priceInfo = appTitles.map((title, index) => `${title}: ${appPrices[index]}`).join('\n');
    }

    if (priceInfo) {
        notify(`应用 ${appName} (${purchaseType}) 的价格信息:\n${priceInfo}`);
    } else {
        notify(`未能获取应用 ${appName} (${purchaseType}) 的价格信息`);
    }
